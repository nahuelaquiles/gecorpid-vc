...
export async function POST(req: NextRequest) {
  try {
    ...
    // 5) Crear QR (PNG) e incrustarlo en el PDF (primera página, esquina inferior derecha)
    const qrPng = await QRCode.toBuffer(verificationUrl, { errorCorrectionLevel: 'M' });
    ...
    // 7) Registrar en DB y descontar crédito
    ...
    // 8) Devolver el PDF procesado para descarga directa
